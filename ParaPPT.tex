...
%comando para paréntesis ajustables
\newcommand{\bc}[1]{
\left(#1\right) }
...
que tiende a 0 en tanto $n \rightarrow \infty$ y $m/n \rightarrow 0$.
...
Note que esto tiende a 0 en tanto $m \rightarrow \infty$.
...
Una forma alternativa de describir los m\'etodos de suavizamiento es utilizando conocimiento respecto a la funci\'on de autocovarianza de la serie. 

Partiendo \ref{ref:suav} y utilizando \ref{eq:1} se tiene 

\begin{eqnarray*}
\bar{f}(\omega) & = & \sum_{k = -m}^{m} W_m(k) I(\omega + k/n)\\
 & = & \sum_{k = -m}^{m} W_m(k) 
 \left[
 \sum_{h = -n+1}^{n-1} \hat{\gamma}(h) e^{-2 \pi i (\omega_j + \frac{k}{n}) h}
 \right]\\
 & = & \sum_{h = -n+1}^{n-1} \hat{\gamma}(h) 
 \left[
 \sum_{k = -m}^{m} W_m(k) e^{-2 \pi i  \;\frac{k}{n} \;h}
 \right] e^{-2 \pi i \omega_j h}\\
\end{eqnarray*}

A partir de lo anterior, se define la funci\'on \textit{lag window} por

\begin{equation}\label{eq:lag_win}
w(x) = \sum_{k = -m}^{m} W_m(k) e^{-2 \pi i k \;x}
\end{equation}

Note que $w(x)$ satisface las siguientes propiedades:

$$
\left\{\begin{matrix}
w(x) &=& w(-x) & &\\ 
w(0) &=& 1 & &\\ 
w(x) &\leq& 1 & & \textup{si }|x|\leq 1
\end{matrix}\right.
$$
...
\begin{itemize}
\item[$\bullet$] \textbf{Daniell modificado: } Consiste en 
\item[$\bullet$] \textbf{Dirichlet: }
La funci\'on \textit{lag window}  est\'a dada por $w(x) = 1$, para $|x| \leq 1$.
\item[$\bullet$] \textbf{Fejer: }
La funci\'on \textit{lag window}  est\'a dada por $w(x) = 1$, para $|x| \leq 1$.
\end{itemize}
